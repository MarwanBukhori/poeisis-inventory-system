  ...
  <center>
    ...
    <form action="orders.php" method="post">
      ...
      try {
        ...
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_staffs_a174856");
        $stmt->execute();
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_customers_a174856");
        $stmt->execute();
        $result = $stmt->fetchAll();
      }
      catch(PDOException $e){
    ...
    <table border="1">
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM tbl_orders_a174856, tbl_staffs_a174856, tbl_customers_a174856 WHERE ";
        $sql = $sql."tbl_orders_a174856.fld_staff_num = tbl_staffs_a174856.fld_staff_num and ";
        $sql = $sql."tbl_orders_a174856.fld_customer_num = tbl_customers_a174856.fld_customer_num";
